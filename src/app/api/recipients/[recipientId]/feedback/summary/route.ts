...
export async function GET(
  ...
  const { data: feedbackRows, error } = await supabase
    ...
    .select("id, preference, suggestion_id, created_at")
    ...
    suggestionMap = typedRuns.reduce<Record<string, SuggestionPayload | null>>(
      (acc, run) => {
        const suggestionsArray = Array.isArray(run.suggestions)
          ? run.suggestions
          : [];
        const first = (suggestionsArray[0] as SuggestionPayload | undefined) ?? null;
        acc[run.id] = first;
        return acc;
      },
      {},
  ...
  (feedbackRows ?? []).forEach((row) => {
    const suggestion = suggestionMap[row.suggestion_id] ?? {};
    const idea = {
      id: row.id,
